...
# The chart stores the generated password in the <app>-auth Secret (present when
# authEnabled, the default). Empty when auth is disabled.
redis_pw() {
    kubectl -n "$NAMESPACE" get secret "$1-auth" -o jsonpath='{.data.password}' 2>/dev/null | base64 -d
}

# Run a redis-cli command against an application's CURRENT master. Master
# discovery goes through the operator's sentinel Service (rfs-<app>, master name
# "mymaster"), so a write always lands on the writable node even after a
