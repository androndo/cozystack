...
# egress (the tenant chart's `<tenant>-egress` CiliumClusterwideNetworkPolicy):
# a worker's importer may reach the outside world and kube-dns, but not an
...
_talos_image_cache_reachable_from_tenant() {
  ...
  rel=$(kubectl -n kube-system exec "$pod" -c serve -- sh -ec \
    ...
    2>/dev/null) || return 1
  [ -n "$rel" ] || return 1
  # Reuse the mirror's own (pre-pulled, digest-pinned) image for the probe Pod so
  # it needs no fresh pull and is guaranteed present on the node.
  ...
  # --retry rides out the few seconds Cilium needs to program the freshly-applied
  # allow policy; a persistently unreachable ClusterIP still fails fast per attempt
  # (connect-timeout) and returns a non-206, falling back to the public factory.
  curl_cmd="curl -s -o /dev/null -w code=%{http_code} --retry 5 --retry-all-errors --retry-delay 3 --connect-timeout 5 --max-time 25 -r 0-0 ${TALOS_IMAGE_CACHE_SVC_URL}/${rel}"
  overrides=$(printf '{"spec":{"securityContext":{"runAsNonRoot":true,"runAsUser":1000,"seccompProfile":{"type":"RuntimeDefault"}},"containers":[{"name":"c","image":"%s","command":["sh","-ec","%s"],"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]}}}]}}' \
    "$img" "$curl_cmd")
