...
for profile in $PROFILES; do
  ...
  # CVE-2026-53359 (Januscape): bake kvm_intel/kvm_amd nested=0 into the boot
  # assets. On UEFI/systemd-boot the kernel cmdline lives in the UKI, so a
  # machine-config `extraKernelArgs` is ignored -- the args must be baked at
  # image build. The imager rejects kernel-arg customization for the bare
  # kernel/initramfs outputs, so only the boot-producing profiles carry it.
  case "$kind" in
    kernel|initramfs)
      customization=""
      ;;
